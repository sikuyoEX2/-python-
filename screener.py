...
def screen_stocks_parallel(stock_list: List[str], max_price: float = 10000, 
    ...
    """
    銘柄を並列スクリーニング（PC向け高速版）
    
    Args:
        stock_list: スキャン対象の銘柄リスト
        max_price: 最大株価
        max_workers: 並列ワーカー数
...
def screen_stocks(stock_list: List[str] = None, max_price: float = 10000, 
    ...
    Args:
        stock_list: スキャン対象の銘柄リスト（Noneの場合は全銘柄）
        max_price: 最大株価
        progress_callback: 進捗コールバック関数
...
def render_screener_page():
    ...
    # スクリーニング実行
    if st.button(f"🚀 スクリーニング開始（{scan_count}銘柄）", type="primary", use_container_width=True):
        ...
        def update_progress(current, total, ticker):
            ...
            status_text.text(f"スキャン中: {ticker} ({current}/{total})")
        
        with st.spinner("銘柄をスキャン中..."):
    ...
    if 'screening_results' in st.session_state:
        ...
        # 対象銘柄プレビュー
        with st.expander("📋 対象銘柄一覧", expanded=False):
            st.markdown("楽天証券かぶミニ対応の主要銘柄をスキャンします：")
            cols = st.columns(4)
            for i, ticker in enumerate(RAKUTEN_MINI_STOCKS[:20]):
                cols[i % 4].caption(ticker)
            if len(RAKUTEN_MINI_STOCKS) > 20:
                st.caption(f"...他 {len(RAKUTEN_MINI_STOCKS) - 20} 銘柄")
...
def render_stock_card(stock: Dict, show_signal: bool = True):
    ...
    with st.container():
        ...
        # 詳細分析へのリンク
        if st.button(f"📊 詳細分析", key=f"analyze_{stock['ticker']}"):
